...
impl<'a,S,M> Clone for &'a MinimaxDelegate<S,M> {
    fn clone(&self) -> &'a MinimaxDelegate<S,M> {
        *self
    }
}
...
pub fn minimax_alpha_beta<'a,S,M  : fmt::Show>(delegate: Box<MinimaxDelegate<S,M>>, state : &'a mut S)  -> (M, int) {
